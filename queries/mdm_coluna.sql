...
daily AS (
    SELECT
        ...
        JSON_OBJECTAGG(KEY hhmi VALUE fa_interval ABSENT ON NULL RETURNING CLOB) AS fa_interval,
        JSON_OBJECTAGG(KEY hhmi VALUE ra_interval ABSENT ON NULL RETURNING CLOB) AS ra_interval,

        JSON_OBJECTAGG(KEY hhmi VALUE i_l1_avg ABSENT ON NULL RETURNING CLOB) AS i_l1_avg,
        JSON_OBJECTAGG(KEY hhmi VALUE i_l2_avg ABSENT ON NULL RETURNING CLOB) AS i_l2_avg,
        JSON_OBJECTAGG(KEY hhmi VALUE i_l3_avg ABSENT ON NULL RETURNING CLOB) AS i_l3_avg,

        JSON_OBJECTAGG(KEY hhmi VALUE u_l1_avg ABSENT ON NULL RETURNING CLOB) AS u_l1_avg,
        JSON_OBJECTAGG(KEY hhmi VALUE u_l2_avg ABSENT ON NULL RETURNING CLOB) AS u_l2_avg,
        JSON_OBJECTAGG(KEY hhmi VALUE u_l3_avg ABSENT ON NULL RETURNING CLOB) AS u_l3_avg,

        JSON_OBJECTAGG(KEY hhmi VALUE u_l1 ABSENT ON NULL RETURNING CLOB) AS u_l1,
        JSON_OBJECTAGG(KEY hhmi VALUE u_l2 ABSENT ON NULL RETURNING CLOB) AS u_l2,
        JSON_OBJECTAGG(KEY hhmi VALUE u_l3 ABSENT ON NULL RETURNING CLOB) AS u_l3,

        JSON_OBJECTAGG(KEY hhmi VALUE i_instant_l1 ABSENT ON NULL RETURNING CLOB) AS i_instant_l1,
        JSON_OBJECTAGG(KEY hhmi VALUE i_instant_l2 ABSENT ON NULL RETURNING CLOB) AS i_instant_l2,
        JSON_OBJECTAGG(KEY hhmi VALUE i_instant_l3 ABSENT ON NULL RETURNING CLOB) AS i_instant_l3,

        JSON_OBJECTAGG(KEY hhmi VALUE r_q1_interval ABSENT ON NULL RETURNING CLOB) AS r_q1_interval,
        JSON_OBJECTAGG(KEY hhmi VALUE r_q2_interval ABSENT ON NULL RETURNING CLOB) AS r_q2_interval,
        JSON_OBJECTAGG(KEY hhmi VALUE r_q3_interval ABSENT ON NULL RETURNING CLOB) AS r_q3_interval,
        JSON_OBJECTAGG(KEY hhmi VALUE r_q4_interval ABSENT ON NULL RETURNING CLOB) AS r_q4_interval,

        JSON_OBJECTAGG(KEY hhmi VALUE fa ABSENT ON NULL RETURNING CLOB) AS fa_total,
        JSON_OBJECTAGG(KEY hhmi VALUE fa_t1 ABSENT ON NULL RETURNING CLOB) AS fa_t1_total,
        JSON_OBJECTAGG(KEY hhmi VALUE fa_t2 ABSENT ON NULL RETURNING CLOB) AS fa_t2_total,
        JSON_OBJECTAGG(KEY hhmi VALUE fa_t3 ABSENT ON NULL RETURNING CLOB) AS fa_t3_total,
        JSON_OBJECTAGG(KEY hhmi VALUE fa_t4 ABSENT ON NULL RETURNING CLOB) AS fa_t4_total,

        JSON_OBJECTAGG(KEY hhmi VALUE ra_total ABSENT ON NULL RETURNING CLOB) AS ra_total,
        JSON_OBJECTAGG(KEY hhmi VALUE ra_t1_total ABSENT ON NULL RETURNING CLOB) AS ra_t1_total,
        JSON_OBJECTAGG(KEY hhmi VALUE ra_t2_total ABSENT ON NULL RETURNING CLOB) AS ra_t2_total,
        JSON_OBJECTAGG(KEY hhmi VALUE ra_t3_total ABSENT ON NULL RETURNING CLOB) AS ra_t3_total,
        JSON_OBJECTAGG(KEY hhmi VALUE ra_t4_total ABSENT ON NULL RETURNING CLOB) AS ra_t4_total,

        JSON_OBJECTAGG(KEY hhmi VALUE fa_md ABSENT ON NULL RETURNING CLOB) AS fa_md,
        JSON_OBJECTAGG(KEY hhmi VALUE fa_md_t1 ABSENT ON NULL RETURNING CLOB) AS fa_md_t1,
        JSON_OBJECTAGG(KEY hhmi VALUE fa_md_t2 ABSENT ON NULL RETURNING CLOB) AS fa_md_t2,
        JSON_OBJECTAGG(KEY hhmi VALUE fa_md_t3 ABSENT ON NULL RETURNING CLOB) AS fa_md_t3,
        JSON_OBJECTAGG(KEY hhmi VALUE fa_md_t4 ABSENT ON NULL RETURNING CLOB) AS fa_md_t4
    FROM slot_values
